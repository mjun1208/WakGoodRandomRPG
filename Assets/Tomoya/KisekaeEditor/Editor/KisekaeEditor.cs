...
namespace Tomoya
{
	public partial class KisekaeEditor : EditorWindow
	{
		...
		[MenuItem("Tomoya/KiseteNe")]
		public static void ShowWindow()
		{
			EditorWindow.GetWindow(typeof(KisekaeEditor));
		}

		void OnGUI()
		{
			GUILayout.Label("きせかえ支援ツール「キセテネ」", EditorStyles.boldLabel);
			...
			if (PrefabUtility.IsAnyPrefabInstanceRoot(m_dress)) {
				EditorGUILayout.HelpBox("服のオブジェクトを右クリックしてUnpackPrefabしてからセットしてください", MessageType.Error, true);
				return;
			}
			...
			GUILayout.Label("着せる", EditorStyles.largeLabel);

			GUILayout.Label("身体をセットしてください", EditorStyles.miniLabel);
			GUILayout.Label("Armatureやメッシュではなくルートオブジェクトです", EditorStyles.miniLabel);

			m_body = EditorGUILayout.ObjectField("身体", m_body, typeof(Animator), true) as Animator;

			if (m_body != null && !m_body.isHuman)
				EditorGUILayout.HelpBox("身体がHumanoidではありません。FBXのRig設定を確認してください", MessageType.Error, true);

			if (GUILayout.Button("着せる")) {
				if (m_body == null || !m_body.isHuman)
					return;
